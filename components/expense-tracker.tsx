...
export default function ExpenseTrackerComponent() {
  ...
  useEffect(() => {
    const storedExpenses = localStorage.getItem("expenses"); //localStorage can only store data as strings. So, when you save an object or an array of objects (like expenses), it gets converted into a string using JSON.stringify
    if (storedExpenses) {
      setExpenses(
        JSON.parse(storedExpenses).map((expenses: Expense) => ({
          //json.parse converted string(expenses) into its orignal from
          ...expenses,
  ...
  const handleAddExpense = () => {
    setExpenses([
      ...expenses, //this is the spread operator, which creates a new array that includes all the existing expenses. You are preserving the current list of expenses while adding a new one.
      {
        id: expenses.length + 1, //The new expense is given a unique id based on the current length of the expenses array.
        name: newExpense.name, //Uses the name value from the newExpense state, which holds the user input for the expense name.
  ...
  const handleDeleteExpense=(id:number):void => {
    setExpenses(expenses.filter((expense)=>expense.id !==id))  //the condition is expense.id !== id, which means it will include only the expenses whose id does not match the id of the expense that you want to delete.
  ...
  const totalExpenses = expenses.reduce(
   ...
   const handleInputChange=(e:ChangeEvent<HTMLInputElement>):void => {
    ...
    setNewExpense((prevExpense)=>({
      ...prevExpense,       //The spread operator ensures that all the existing properties of newExpense (e.g., name, amount, date) remain intact.
      [id]:           //The square brackets allow dynamically updating the property of the object using the id of the input field.
      id==="amount"
      ? parseFloat(value)
      : id==="date"?
      new Date(value)
      : value   //For all other cases (like name), the value remains a string.
    }))
